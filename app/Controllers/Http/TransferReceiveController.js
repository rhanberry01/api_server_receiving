...
class TransferReceiveController {
    ...
    async post_receiving({ request, response }) {
        ...
        let { p_remarks, p_transfer_no } = request.only(['p_remarks', 'p_transfer_no'])
        ...
        let receiveArrayQty = []
        for(const row of product_id_list) {
          product_id_array.push(row.prod_id)
          receiveArrayQty.push(row.qty)
        }

        const receiveQty = receiveArrayQty.reduce((prev, current) => {
            return prev + current
        })
        const qty = await TransferReceiveMod.check_qty_receive_item(p_transfer_no)
        let qty_ = parseFloat(qty)
            qty_ =  qty_.toFixed(2)
        let receiveQty_ = parseFloat(receiveQty)
            receiveQty_ =  receiveQty_.toFixed(2)

        if(qty_ !== receiveQty_) {
          throw new CustomException({ message: `KUNG ANO ANG NA DISPATCHED YUN DIN ANG MARERECEIVED`  }, 401)
        }
